...
import { getAchievementsByWeekdayOslo, getBestStreakOslo, getCumulativeAchievementsByMonthOslo, getDashboardSummary, getGameProgress, getGameUnlockStats, getRecentAchievements, getTopGames, getUnlocksByDay, } from "./repositories/dashboard.js";
 ...
 */
function registerDebugRoutes(fastify) {
    /**
    ...
    fastify.post("/debug/steam/poll-once", async () => {
        const summary = await syncSteamAchievementsForConfiguredUser();
        const newEvents = summary.newEventsSample;
...
async function start() {
    ...
    app.log.info({ intervalSec }, "Poller enabled: will sync + send webhooks for new achievements.");
    const runPollOnce = async () => {
        try {
            ...
            app.log.info({
                ...
                newUnlocksInserted: summary.newUnlocksInserted,
