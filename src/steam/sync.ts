...
export async function syncSteamAchievementsForConfiguredUser() {
  ...
  if (!steamUserId) throw new Error("Missing STEAM_STEAMID64 in environment.");
  ...
  const perAppInsertedSample: Array<{ appid: number; inserted: number }> = [];

  // Collected per sync run (not global) so the poller can notify only for new rows.
  const newEvents: NewAchievementEvent[] = [];
  ...
  for (const g of selected) {
    ...
    try {
      ...
      for (const a of unlocked) {
        ...
        const event: NewAchievementEvent = {
          steamUserId,
          appId: g.appid,
